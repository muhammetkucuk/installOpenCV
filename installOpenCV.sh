...
sudo apt-get install libavdevice-dev libavresample-dev libass-dev libfdk-aac-dev libfreetype6-dev libopus-dev libvpx-dev libx265-dev -y  &&
...
mkdir build &&
cd build &&
cmake .. -DCMAKE_INSTALL_PREFIX=/usr/local -DWITH_QT=ON -DWITH_V4L=ON -DWITH_OPENGL=ON -DWITH_TBB=ON -DOPENCV_EXTRA_MODULES_PATH=../../opencv_contrib-3.4.2/modules &&
make -j4 &&
sudo make install &&
cd ../.. &&
